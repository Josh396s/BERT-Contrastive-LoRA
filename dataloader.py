...
class BaseInstance(object):
    def __init__(self, embed_data, example):
        ...
        # labels, ensure labels are tensors
        self.scenario_label = torch.tensor(example['label']) 
...
class ScenarioDataset(Dataset):
    ...
    def collate_func(self, batch):
        input_ids = torch.stack([f.embedding for f in batch], dim=0)
        segment_ids = torch.stack([f.segments for f in batch], dim=0)
        input_masks = torch.stack([f.input_mask for f in batch], dim=0)
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            'input_ids': instance.embedding.clone().detach().long(),
            'attention_mask': instance.input_mask.clone().detach().long(),
            'labels': torch.tensor(instance.scenario_label, dtype=torch.long)  # Correct attribute name
        }